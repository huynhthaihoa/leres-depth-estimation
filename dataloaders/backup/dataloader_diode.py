...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            if self.min_col_bound is None:
                self.min_col_bound, self.min_row_bound = image.size
                self.min_row_bound -= self.args.input_height
                self.min_col_bound -= self.args.input_width
            width_index = random.randint(0, self.min_col_bound) # int(np.random.rand() * self.min_col_bound)
            height_index = random.randint(0, self.min_row_bound) # int(np.random.rand() * self.min_row_bound)
