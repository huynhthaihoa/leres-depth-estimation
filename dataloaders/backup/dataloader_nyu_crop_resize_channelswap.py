...
from ._functional import preprocessing_transforms
# seed = 42
...
class NewDataLoader(object):
    def __init__(self, args, mode):
        ...
        if mode == 'train':
            self.training_samples = DataLoadPreprocess(args, mode, transform=preprocessing_transforms(mode, args.normalize))
            if args.distributed:
...
class DataLoadPreprocess(Dataset):
    def __init__(self, args, mode, transform=None, is_for_online_eval=False):
        ...
        if self.mode == 'train':                          
            self.input_width_original, self.input_height_original = 544, 416
            self.min_width_index = 43
            self.max_width_index = 608 #- self.input_width_original #+ 1
            self.min_height_index = 45
            self.max_height_index = 472 #- self.input_height_original #+ 1
        # else:
    ...
    def __getitem__(self, idx):
        if self.mode == "test":
            ...
            depth_gt =  Image.open("{}/{}".format(self.args.data_path, depth_path))
        ...
        if self.mode == "train":
            ...
            depth_gt = depth_gt.crop((self.min_width_index, self.min_height_index, self.max_width_index, self.max_height_index))
            
            image = image.resize((self.input_width, self.input_height), Image.BILINEAR)
            depth_gt = depth_gt.resize((self.input_width, self.input_height), Image.NEAREST)
            ...
            image = image.resize((self.input_width, self.input_height), 0)
            depth_gt = depth_gt.resize((self.input_width, self.input_height), 0)
    ...
    def augment_image(self, image):
        ...
        image_aug = image_aug[..., list(indices[policy_idx])]
...
class ToTensor(object):
    def __init__(self, mode):
        ...
        # self.resize = transforms.Resize([args.input_height, args.input_width])
