...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            image_path = os.path.join(self.args.data_path, 'raw_dataset', rgb_file)
            depth_path = os.path.join(self.args.data_path, 'train', depth_file)
            ...
            image_path = os.path.join(data_path, "raw_dataset/" + sample_path.split()[0])
    ...
    def train_augment(self, image, depth_gt):
        # Depth shift
        random_shift = np.random.uniform(-1, 1)
        depth_gt = self.pixel_shift(depth_gt, random_shift)
    ...
    def pixel_shift(self, depth, shift):
        depth = depth + shift
        return depth
