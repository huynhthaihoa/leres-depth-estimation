...
class NyuShiftDataLoader(BaseDataLoader):
    def __init__(self, args, mode):
        super().__init__(args, mode)
        
        dataset = NyuShiftDataset(args, mode, transform=self.transform)
        # if mode == "train":
        #     self.training_samples = NyuShiftDataset(args, mode, transform=self.transform)
        #     # if args.distributed:
        ...
        # elif mode == "eval":
        #     self.testing_samples = NyuShiftDataset(args, mode, transform=self.transform)
        #     # if args.distributed:
...
class NyuShiftDataset(BaseDataset):
    def __init__(self, args, mode, transform=None):#, is_for_online_eval=False):
        ...
        if self.mode == "train":
            ...
            self.input_width = ((self.max_width_index - self.min_width_index) // 32 + 1) * 32
            
            self.min_height_index = 45
            self.max_height_index = 472 #- self.input_height #+ 1
            self.input_height = ((self.max_height_index - self.min_height_index) // 32 + 1) * 32
    ...
    def __getitem__(self, idx):
        if self.use_cache and idx in self.caches.keys():
            ...
            if self.mode == "train":
                ...
                # apply cutdepth
                # image_pad = self.augment_cutdepth(image_pad, depth_gt_pad)
