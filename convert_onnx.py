...
if __name__ == '__main__':
    ...
    # Output setup
    parser.add_argument('-o', '--output_path',                 type=str,   help='output ONNX file path', default='')
    ...
    sample_input = torch.rand((1, 3, args.input_height, args.input_width))
    
    output_path = args.output_path
    
    if output_path == '':
        if args.weight_path.endswith("ckpt"):
            output_path = args.weight_path.replace("ckpt", "onnx")
        elif args.weight_path.endswith("pth"):
            output_path = args.weight_path.replace("pth", "onnx")
    ...
    logger.info(f"Converting MetricLeReS with encoder {args.encoder}, decoder {args.decoder}, feature list {args.feature_list} to ONNX successfully! Output path is {output_path}")
