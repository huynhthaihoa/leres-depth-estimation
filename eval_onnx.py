...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='MetricLeReS ONNX evaluation', fromfile_prefix_chars='@')
    ...
    try:
        model = ONNXDepthModel(weight_path=args.weight_path, use_cuda=True, normalize=args.normalize, use_tta=args.use_tta, original_scale=args.original_scale)
        params = calculate_params(onnx.load_model(args.weight_path))
        
        logger.info(f'== Loaded ONNX model {args.weight_path} successfully!. Number of params: {params}')

        if args.log_dir == '':
            args.log_dir = args.weight_path.replace(".onnx", "_onnx")
        ...
        logger.error(f"== Loaded ONNX model {args.weight_path} failed! Please check it again")
    ...
    for idx in tqdm(range(num_samples)):
        ...
        # predicted depth
        pred_depth = model(image, args.to_grayscale)
        ...
        disp_image_path = "{}/{}_{}".format(image_output_dir, tokens[-2], tokens[-1])
