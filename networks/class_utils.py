...
class FeatureFusionDeconvBlock(nn.Module):
    """Feature fusion block.
    """

    def __init__(self, features, activation, bn=False, expand=False):#, deconv=False, size=None):
        """Init.

        Args:
            features (int): number of features
        """
        super(FeatureFusionDeconvBlock, self).__init__()
        ...
        # self.size=size
        self.upsample = nn.ConvTranspose2d(in_channels=features, out_channels=features, kernel_size=2, stride=2)               
...
class NeWCRFsDeconvDispHead(nn.Module):
    def __init__(self, input_dim=100, upscale_factor=2):
        super(NeWCRFsDeconvDispHead, self).__init__()
        self.conv1 = nn.Conv2d(input_dim, 1, 3, padding=1)
        self.upsample = nn.ConvTranspose2d(in_channels=1, out_channels=1, kernel_size=upscale_factor, stride=upscale_factor)
        self.sigmoid = nn.Sigmoid()
...
def _make_fusion_deconv_block(features, use_bn):
    return FeatureFusionDeconvBlock(
        features,
        nn.ReLU(False), 
        bn=use_bn,
        expand=False      
