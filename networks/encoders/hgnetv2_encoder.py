...
class HGNetv2(BaseEncoder):
    ...
    def __init__(self,
        ...
        if architecture.startswith('cocoobject365') is True and 'url_cocoobject365' in self.arch_configs[name].keys():
            ...
            is_stage_backbone = False
        elif architecture.startswith('deim') is True and 'path_deim' in self.arch_configs[name].keys():
            model_path = self.arch_configs[name]['path_deim']
            is_stage_backbone = False
        ...
        if pretrained:
            if architecture.startswith('deim') is True:
                print("model path:", model_path)
                try:
                    
                    state = torch.load(model_path, map_location='cpu', weights_only=False)
                    logger.info(f"Loaded {model_path} from local file.")
                except:
                    logger.error(f"Cannot load {model_path} from local file.")
            else:
