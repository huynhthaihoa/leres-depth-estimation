...
def inject_lora_into_model(model, r=8, target_module_name=None, alpha=1, dropout=0., replace=False, verbose=False, finetune_bias=False):#="attention"):
    for name, module in model.backbone.named_modules():
        ...
        if type(module) is nn.Linear and (target_module_name is None or (target_module_name is not None and target_module_name in name)):
            ...
            parent = _get_parent_module(model.backbone, name)
