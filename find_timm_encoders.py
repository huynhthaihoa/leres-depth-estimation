...
for model_name in model_names:
    try:
        ...
        if len(features) >= 4:
            logger.info(f'{model_name} supports up to {len(features)}-level features:')
        else:
            logger.warning(f'{model_name} supports up to {len(features)}-level features:')
        for i, feature in enumerate(features):
            logger.info(f"      feature {i + 1} has shape: {feature.shape}")
    except:
        logger.warning(f'{model_name} does not support feature extractor')