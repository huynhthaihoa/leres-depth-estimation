...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='TFLite model evaluation (multiple models).', fromfile_prefix_chars='@')

    parser.add_argument('-c', '--config', help="configuration file *.yml", type=str, required=False, default='')

    # Input dir
    parser.add_argument('-id', '--input_dir',           type=str,   help='Directory contains TFLite models', required=False)
    ...
    weight_paths = glob(f"{args.input_dir}/**/*.tflite", recursive=True) #+ glob(f"{args.input_dir}/*/*.onnx")
    ...
    for weight_path in weight_paths:
        ...
        command = f"python eval_tflite.py -c {config_path}"
